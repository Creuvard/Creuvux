...
cb_sax_start_document( void *user_data)
{
	...
	fprintf(output, "%s", "<?xml-stylesheet type=\"text/xsl\" href=\"/web/dl.xsl\"?>\n");
...
int WebUI_dl(const char *sha1_dl, const char *eta_dl, const char *title_dl, float pct_dl, float rate_dl)
{
	char path[SIZE];
	FILE *fd;
	...
	//fprintf(stderr, "%s %s %s %f %f\n", sha1_dl, eta_dl, title_dl, pct_dl, rate_dl);
	...
	fprintf(fd, "%s %s %s %f %f", sha1_dl, eta_dl, title_dl, pct_dl, rate_dl);
	...
	(void)crv_strncat(path, "download.xml", sizeof(path));

	(void)crv_strncpy(repport, options.path, sizeof(repport));
	(void)crv_strncat(repport, "/web/", sizeof(repport));
	(void)crv_strncat(repport, "repport.xml", sizeof(repport));
